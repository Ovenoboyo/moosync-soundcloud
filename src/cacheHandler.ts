...
export class CacheHandler {
  ...
  public async addToCache(url: string, data: string) {
    try {
      if (!this.tryJson || (this.tryJson && JSON.parse(data))) {
        const expiry = Date.now() + 0.5 * 60 * 60 * 1000
  ...
  public getParsedCache<T>(url: string): T | undefined {
    try {
      return JSON.parse(this.getCache(url))
    } catch (e) {
      console.debug('Failed to parse cache for', url)
    }
  }
